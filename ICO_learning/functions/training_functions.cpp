...
void train_roll(Motor left, Motor right, matrix_hal::IMUData imu_data, float weight_roll[], float learning_rate, int speed[], matrix_hal::GPIOControl gpio, matrix_hal::IMUSensor imu_sensor, float limit, int update_method)
{
    ...
    while (true){
        ...
        file << weight_roll[0] << "," << weight_roll[1] << "," << imu_data.roll << "," << mean_roll << "," << speed[0]+extra[0] << "," << speed[1]+extra[1] << "," << reflex << "," << std::chrono::duration_cast<std::chrono::milliseconds>(start - learning_start).count() << std::endl;
        ...
        std::this_thread::sleep_for(std::chrono::duration_cast<std::chrono::milliscendos>( sampling_time - std::chrono::duration_cast<std::chrono::milliseconds>(finish - start).count()));

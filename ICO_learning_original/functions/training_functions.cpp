...
void Run2(float weight_roll[], float weight_pitch[] ,Motor left, Motor right, matrix_hal::IMUData imu_data, matrix_hal::GPIOControl gpio, matrix_hal::IMUSensor imu_sensor, float sampling_time, float cutoff, int speed[])
{
    ////////////////////////////////////////////////////////////////////////
    // Initialization of variables
    int duration = 10000;
...
void TrainBoth2(Motor left, Motor right, matrix_hal::IMUData imu_data, float weight_roll[], float weight_pitch[], float learning_rate, int speed[], matrix_hal::GPIOControl gpio, matrix_hal::IMUSensor imu_sensor, float limit, float sampling_time, float cutoff, int * iteration)
{
    //Variables required for the different calculations:
    float bias_roll, bias_pitch;
    float mean_roll, mean_pitch;
    ...
    *iteration++;
    ...
    while (true){
        ...
        file << 0 << "," << weight_roll[0] << "," << weight_roll[1] << "," << weight_pitch[0] << "," << weight_pitch[1] << "," << imu_data.roll << "," << mean_roll << "," << imu_data.pitch << "," << mean_pitch << "," << speed[0]+extra[0] << "," << speed[1]+extra[1] << "," << reflex << ","  << reflex_ON << ',' << *iteration<< std::endl;
